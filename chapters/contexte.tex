...
Aboard Engineering fournie différents types de produits et services. Je ne vais ici développer que le cas de la plate-forme \gloss{orianne} sur laquelle j'ai travaillé.
...
Pour cela, une interface graphique permet de configurer quelles fonctions sont à intégrer dans l'application (elles-mêmes configurables) mais aussi les caractéristiques du moteur cible ou la configuration du \gloss{rtos} (définition des récurrence des tâches).
...
L'entreprise possède également plusieurs ordinateur portable sur lesquels sont installés certains logiciels comme Matlab\up{\circledR}, \gloss{inca} ou \gloss{canalyzer}. Ces ordinateurs portables sont en libre service aux employés pour l'utilisation des ces logiciels en particuliers ou simplement pour avoir un environnement de test indépendant de l'environnement de développement.
